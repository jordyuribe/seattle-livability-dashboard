...
async function refreshData() {
  ...
  const [sensors, parksGeoJSON] = await Promise.all([
    fetchAirQuality(),
    fetchGreenSpace()
  ]);
  ...
  neighborhoodGeoJSON.features.forEach(feature => {
    ...
    const score = computeLivabilityScore({
      aqi,
      noiseDb: 60,  // placeholder — replace when noise API is connected
    ...
    scores[name] = { aqi, score, greenPct, parkCount: parkData ? parkData.count : 0 };
  });
